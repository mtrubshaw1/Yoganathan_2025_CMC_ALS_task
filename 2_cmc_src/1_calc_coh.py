...
for task in tasks:
    ...
    for n, subject in enumerate(subjects):
        ...
        for ch in range(len(channels)):
            motor_.append(meg_sub[:,[4,30]])
        motor= np.squeeze(np.array(motor_))
        
        emg_sub = data_emg[n]
        emg_ = []
        for ch in range(len(emg_ch)):
            emg_.append(emg_sub[:,[0,1]])
        time = np.arange(motor.shape[2])/250
        emg = np.squeeze(np.array(emg_))
        ...
        motor_bl = emg_motor_bl[:2]
        emg_bl = abs(emg_motor_bl[2:])
...
np.save('data/cohs_tasks.npy',cohs_tasks)
