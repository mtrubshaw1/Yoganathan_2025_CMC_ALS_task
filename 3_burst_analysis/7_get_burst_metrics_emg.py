...
for n, subject in enumerate(subjects):
    epoch_file_meg = epoch_dir / f'{task}_s0{subject}_emg_grip_epo.fif'
    epochs_meg = mne.read_epochs(epoch_file_meg)
    d_meg = epochs_meg.get_data()
    if task == 'task1':
        if missing_task2[n] == 'No' or missing_task2[n] == 'Mri':
            epoch_file_meg = epoch_dir / f'task2_s0{subject}_emg_grip_epo.fif'
            epochs_meg = mne.read_epochs(epoch_file_meg)
...
for sub in range(len(data_meg)):
    ...
    motor_r = np.concatenate(data[:,0,:],axis=0)
    motor_l =np.concatenate(data[:,1,:],axis=0)
    

    is_burst_r, _ = burst_detection(motor_r, freq_range, fsample = fsample, #normalise = 'mean', 
                                                  threshold_dict = {'Method': 'Percentile', 'threshold': 75}, min_n_cycles = 1)
    
    
    is_burst_l, _ = burst_detection(motor_l, freq_range, fsample = fsample, #normalise = 'mean', 
                                                  threshold_dict = {'Method': 'Percentile', 'threshold': 75}, min_n_cycles = 1)
    ...
    tc = is_burst_.reshape(data[:,0,:].shape)
    is_bursts.append(tc)
...
toi = np.arange(1200,1300)#,np.arange(1200,1500)
# toi = np.hstack((toi[0],toi[1]))
beta_rest = np.mean(np.squeeze(beta_tc_unstd[:,:,[toi]])[:,[4,30]],axis=(1,2))

# calculate beta power in rest period (1-5s) (baselined)
...
# np.save('data/is_bursts_emg.npy',is_bursts)
