...
betas = np.load('data/psd_tf_betas_range_emg.npy')
...
#%%plot beta tf
beta_tc = np.mean(betas[:,:2],axis=1)


time = np.arange(beta_tc.shape[1])/250

beta_tc_bl = mne.baseline.rescale(beta_tc,time,[0,6])
# beta_tc_bl= beta_tc
...
# plt.title('EMG beta power modulation (baseline corrected)')
...
plt.savefig('plots/beta_power_tf_bl_emg', dpi=300)
plt.show()
