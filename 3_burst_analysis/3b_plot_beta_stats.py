...
means = np.vstack((beta_rest,beta_15)).T
...
group_labels = ["Raw beta power \nat rest",
                "Baseline corrected \nbeta power \n(averaged between 1-3s)"]
...
for i in range(n):
    if pvalues[i] < 0.001:
        ...
        plt.text(i, values[i], '*', ha='center', va='baseline', color='red', fontsize=24)
# plt.tight_layout()
plt.title('Beta power metrics (ALS-HC)',fontsize=18)
plt.savefig(f"plots/beta_power_stats.png",dpi=300)
